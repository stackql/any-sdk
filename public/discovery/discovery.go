...
func (adp *BasicDiscoveryAdapter) GetServiceShard(
	...
	serviceIDString := docparser.TranslateServiceKeyGenericProviderToIql(serviceKey)
	...
	viewCollection, viewCollectionPresent := rsc.GetViewsForSqlDialect(adp.persistenceSystem.GetSystemName())
	// TODO: add AOT view analysis or abstraction for view creation
	if viewCollectionPresent {
		adp.persistenceSystem.HandleViewCollection(viewCollection)
	}
	return shard, nil
}
...
func (adp *BasicDiscoveryAdapter) PersistStaticExternalSQLDataSource(prov anysdk.Provider) error {
	...
	err := adp.persistenceSystem.HandleExternalTables(providerName, externalTables)
	if err != nil {
		return err
	}
	return nil
...
func (store *TTLDiscoveryStore) processResourcesDiscoveryDoc(
	...
	case "googleapis.com", "google":
		...
		err = store.persistenceSystem.CacheStorePut(k, bt, "", 0)
		if err != nil {
			return nil, err
		}
		return rr, err
