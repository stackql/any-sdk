...
implemented in C (github.com/stackql/sqlite-ext-functions and the
stackql-go-sqlite3 fork's amalgamation additions, both retired for the
embedded engine). Registered unconditionally into the modernc.org/sqlite
embedded engine in public/sqlengine - every any-sdk consumer gets these
functions; there is no build tag.
...
public/sqlfuncs/
  ...
  testdata/           - golden vectors: sqlite-ext-functions tests + cgo-build
                        samples captured in stackql PR #783
...
- Every function has a native Go fuzz target (FuzzSplitPart, FuzzRegexpLike,
  ...
  as regression cases.
- Unit tests run without a database. One integration test in the sqlengine
  package proves registration end to end via SQL, one call per function.
...
4. Document it in the function reference docs (any-sdk and stackql).
