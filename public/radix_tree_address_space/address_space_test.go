...
func TestBasicAddressSpaceGoogleCurrent(t *testing.T) {
	...
	mappedNamsespace, mapErr := addressSpace.ToMap(radix_tree_address_space.NewStandardAddressSpaceExpansionConfig(false, true, true))
	if mapErr != nil {
...
func TestHeavyAliasedAddressSpaceGoogleCurrent(t *testing.T) {
	...
	addressSpace, _ := method.GetAddressSpace()
	if addressSpace == nil {
		t.Fatalf("Address space analysis failed: expected non-nil address space")
	}
	requestBody, requestBodyOk := addressSpace.DereferenceAddress("request.body")
...
func TestHeavyAliasedAddressSpaceAWSCurrent(t *testing.T) {
	registryLocalPath := "./testdata/registry/basic"
	localRegistry, registryErr := getNewTestDataMockRegistry(registryLocalPath)
	if registryErr != nil {
		t.Fatalf("Failed to create mock registry: %v", registryErr)
